...
public class Final_script : MonoBehaviour {
	...
	void Update () {		
		...
		// Flexing fingers
		thumb_angle = -((thumb_flex_start - thumb_flex) / (thumb_flex_start - thumb_flex_end) * 90);
		...
		index_angle =  - ((middle_flex_start - middle_flex) / (middle_flex_start - middle_flex_end) * 90);
		if (middle_angle < -90) index_angle = -90;
		if (middle_angle > 0) index_angle = 0;
		index_start.localRotation = Quaternion.Euler(middle_start.localRotation.eulerAngles.x, middle_start.localRotation.eulerAngles.y, start_position_middle + middle_angle);
		index_mid.localRotation = Quaternion.Euler(0, 0,middle_angle);
		index_final.localRotation = Quaternion.Euler(0, 0,middle_angle);
	...
	private void data_receiving_thread()
	{
		...
		while (true)
		{
			...
			try
			{
				thumb_flex = float.Parse(dataString[0]);
				index_flex = float.Parse(dataString[1]);
				middle_flex = float.Parse(dataString[2]);
				ring_flex = float.Parse(dataString[3]);
				pinky_flex = float.Parse(dataString[4]);
				q0 = float.Parse(dataString[5], CultureInfo.InvariantCulture.NumberFormat);
				q1 = float.Parse(dataString[6], CultureInfo.InvariantCulture.NumberFormat);
				q2 = float.Parse(dataString[7], CultureInfo.InvariantCulture.NumberFormat);
				q3 = float.Parse(dataString[8], CultureInfo.InvariantCulture.NumberFormat);
			}
